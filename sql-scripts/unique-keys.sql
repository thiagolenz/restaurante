...
CREATE INDEX UK_LOCATIONS_ENGLISH ON Locations (nameEnglish);
CREATE INDEX UK_LOCATIONS_PORTUGUESE ON Locations (namePortuguese);
CREATE INDEX UK_LOCATIONS_SPANISH ON Locations (nameSpanish);


CREATE INDEX UK_RECEIPT ON Receipt (userId);

CREATE UNIQUE INDEX UK_USER_AVATAR ON UserAvatar (userId);
CREATE UNIQUE INDEX UK_ORDER_AVATAR ON OrderAvatar (orderId);